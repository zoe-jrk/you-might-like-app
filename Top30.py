...
# ---------- STEP 5: Build Personalized Interaction Matrix ----------
interaction_data = []
...
# ---------- STEP 9: Batch: All Users × All Moods ----------
all_recommendations = []

for user_internal_idx in range(n_users):
    user_id = train_set.idx2userid[user_internal_idx]
    for mood in ["mood_happy", "mood_sad", "mood_bored"]:
        recommendations = recommend_top_n_mood(user_internal_idx, mood, model, train_set, movies_df, movie_id_to_title, top_n=30)
        for rank, title in enumerate(recommendations, 1):
...
output_path = "all_user_mood_recommendations.csv"
output_df.to_csv(output_path, index=False)

print(f"\n✅ Saved batch recommendations to {output_path}")
